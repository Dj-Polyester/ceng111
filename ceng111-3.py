...
listq[8]="Cache memory is cheaper,faster and smaller in capacity compared to RAM.(T/F) $False"
lista[8]="Cache memory uses a more expensive design."
...
listq[26]=" ... enables a device to transfer data without exposing the CPU to work overload. With ... ,the CPU  can process other tasks while data transfer is being performed. When the transfer is complete, the CPU receives an interrupt signal by ... . $DMA"
p=0
n=len(listq)
for i in range(n):
        no=i+1
